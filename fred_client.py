...
class AgenticMCPClient:
    ...
    async def run_agentic_loop_with_tracking(self, conversation_history: list, max_iterations: int = None) -> Tuple[
        str, List[Dict]]:
        """
        Run the agentic loop and track all tool calls for evaluation.
        Returns: (final_response, list_of_tool_calls)
        """
        if max_iterations is None:
            max_iterations = Config.MAX_ITERATIONS
        ...
        tool_calls_made = []  # Track all tool calls
        iteration = 0
        ...
        while iteration < max_iterations:
            ...
            if message.tool_calls:
                # Capture tool calls BEFORE executing them
                for tool_call in message.tool_calls:
                ...
                return message.content if message.content else "", tool_calls_made

        return "Maximum iterations reached without complete response.", tool_calls_made
